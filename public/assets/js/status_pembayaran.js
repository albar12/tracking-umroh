$(document).ready(function () {
    ...
    var table = $("#datatable").DataTable({
        ...
        ajax: {
            url: "/setting/status-pembayaran/getStatusPembayarans",
            type: "POST",
            data: function (d) {
                // d.filters = {
        ...
        columns: [
            ...
            },
            {
                ...
                "render": function (data, type, row) {
                    ...
                    buttons += `<a href="status-pembayaran/${encodeURIComponent(data)}" class="text-info" title="Lihat Data">
                                            <i class="fa-solid fa-eye font-size-18"></i>
                                        </a>`;
                    buttons += `<a href="status-pembayaran/${encodeURIComponent(data)}/edit" class="text-success" title="Edit Data">
    ...
    $(document).on("click", ".delete-btn", function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: BASE_URL + 'setting/status-pembayaran/' + userId,
                    type: "POST",
                    data: {
                        _method: "DELETE",
