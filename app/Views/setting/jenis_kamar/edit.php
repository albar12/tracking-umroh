    ...
    <div class="row align-items-center mb-4">
        <div class="col-sm-6">
            <h4 class="mb-1 fw-bold text-dark fs-4">
                <i class="fa-solid fa-bed text-success me-2"></i> Form Edit <?= $title ?>
            </h4>
        ...
        <div class="col-sm-6 text-sm-end mt-3 mt-sm-0">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb justify-content-sm-end mb-0 bg-transparent p-0 fs-7">
                    <li class="breadcrumb-item"><a href="<?= base_url('setting/jenis-kamar') ?>" class="text-decoration-none text-secondary"><?= $title ?></a></li>
        ...
        <div class="col-12">
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                <div class="card-body p-4 p-lg-5">
                    ...
                    <form class="needs-validation" action="<?= base_url('setting/jenis-kamar/' . $id) ?>" method="POST" enctype="multipart/form-data" novalidate>
                        <?= csrf_field(); ?>
                                ...
                                <div class="col-md-3">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control number-only py-2 rounded-3" id="kamar" name="kamar" minlength="6"
                                            autofocus required placeholder="Jenis Kamar" value="<?= $jenisKamar['kamar'] ?>">
                                        <div class="invalid-feedback" id="nik-feedback">
                                            Data wajib diisi.
                                        </div>
                                        <input type="hidden" id="kamar_old" name="kamar_old" value="<?= $jenisKamar['kamar'] ?>">
                                    </div>
                                </div>
                                ...
                                <div class="col-md-3">
                                    <div class="mb-3">
                                        ...
                                        <div class="form-check form-switch pt-1">
                                            <input class="form-check-input" type="checkbox" role="switch" id="status" name="status" value="Aktif" style="width: 2.8em; height: 1.4em; cursor: pointer;" <?= (isset($jenisKamar['status']) && $jenisKamar['status'] === 'Aktif') ? 'checked' : '' ?>>
                                            <label class="form-check-input-label fw-semibold ms-2" for="status" id="statusLabel" style="cursor: pointer; line-height: 1.8;">
                                                <?= (isset($jenisKamar['status']) && $jenisKamar['status'] === 'Aktif') ? 'Aktif' : 'Tidak Aktif' ?>
                                            </label>
                                        </div>
                                    </div>
                                </div>
