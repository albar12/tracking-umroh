        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <table id="datatablex" class="table table-bordered dt-responsive">
                                <thead>
                                    <tr>
                                        <th>Produk</th>
                                        <th>Kategori</th>
                                ...
                                <tbody>
                                    ...
                                    foreach ($detailBarangMasuk as $key => $value) :
                                        $total += $value['qty_input'];
                                        $rowClass = (empty($value['qty_input']) && $value['qty_input'] !== '0') ? 'table-danger' : '';
                                    ?>
                                        <tr>
                                            ...
                                            <td><?= esc($value['qty']) ?></td>
                                            <td class="<?= $rowClass ?>"><?= esc($value['qty_input']) ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                                <tfoot>
                                    <th colspan="3" class="text-end">Total Produk</th>
                                    <th><?= esc($barangMasuk['total_produk']) ?></th>
                                    <th><?= esc($total) ?></th>
                                </tfoot>
                            </table>
                        </div>
                    ...
                    <br>
                    <form class="needs-validation" action="<?= base_url('stok/barang-masuk/upload-dokumen') ?>" method="POST" enctype="multipart/form-data" novalidate>
                        <?= csrf_field(); ?>
                        <input type="hidden" name="barang_masuk_id" id="barang_masuk_id" value="<?= $brang_masuk_id ?>">
                        <div class="row">
                            <div class="col-md-3">
                                <div class="mb-3">
                                    <input class="form-control" required type="file" id="gambar" name="gambar" placeholder="Add your item here..." aria-label="Add your item here...">
                                    <div class="invalid-feedback">
                                        Data wajib diisi.
                                    </div>
                                </div>
                            ...
                            <div class="col-md-4">
                                <div class="hstack gap-3">
                                    <button class="btn btn-primary" type="submit" style="float: right">Upload Dokumen</button>
                                </div>
                            </div>
                        </div>
                    </form>
                                                    ...
                                                    <?php if (str_contains($row['dokumen'], "https")): ?>
                                                        <a href="<?= $row['dokumen'] ?>" title="<?= $row['tipe'] ?>">
                                                            <div class="img-fluid m-1">
                                                                <img src="<?= $row['dokumen'] ?>" alt="<?= $row['tipe'] ?>" width="120" height="120">
                                                            </div>
                                                        </a>
                                                    <?php else: ?>
                                                        <a href="<?= base_url($row['dokumen']) ?>" title="<?= $row['tipe'] ?>">
                                                            <div class="img-fluid m-1">
                                                                <img src="<?= base_url($row['dokumen']) ?>" alt="<?= $row['tipe'] ?>" width="120" height="120">
                                                            </div>
                                                        </a>
                                                    <?php endif; ?>

                                                    <button class="text-danger delete-btn-dokumen" data-id="<?= $row['dokumen_id'] ?>" data-file="<?= $row['dokumen'] ?>">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <?php if ($barangMasuk['approval_keterangan']) { ?>
                                Note sebelumnya : <code><?= $barangMasuk['approval_keterangan'] ?></code>
                            <?php } ?>
                        </div>
                    </div>
                    <div class="row mt-4">
                        <div class="col-sm-12">
                            ...
                            <?php } elseif (count($dokumen) == 0) { ?>
                                <span data-bs-toggle="tooltip" style="float: right" data-bs-placement="top" title="Wajib Upload Dokumen">
                                    <button type="button" class="btn btn-primary" disabled>
                                        Simpan
                                    </button>
                                </span>
                            <?php } elseif ($total != $barangMasuk['total_produk']) { ?>
                                <span data-bs-toggle="tooltip" style="float: right" data-bs-placement="top" title="Qty Terima tidak sesuai">
                                    <button type="button" class="btn btn-primary" disabled>
                                        Simpan
                                    </button>
                                </span>
